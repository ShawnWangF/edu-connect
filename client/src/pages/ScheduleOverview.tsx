...
export default function ScheduleOverview() {
  // 支持兩種使用方式：
  // 1. 作為首頁（/），通過項目選擇器選擇項目
  // 2. 作為項目詳情頁（/projects/:projectId/schedule），通過 URL 參數獲取項目 ID
  ...
  const { data: allProjects = [] } = trpc.projects.list.useQuery(undefined, {
    enabled: !urlProjectId, // 只在首頁模式下獲取
  });
  ...
  // 建立 blockMap: groupId_date -> block
  ...
  async function handleGenerateTemplate() {
    ...
    for (let i = 0; i < templateHkDays; i++) {
      ...
      newBlocks.push({ date: d.toISOString().split('T')[0], blockType: isExchange ? 'exchange' : (i === 0 ? 'hk_arrive' : 'hk_stay'), isExchangeDay: isExchange, hotelCity: 'hk' });
      dayOffset++;
    }
    ...
    try {
      ...
      toast.success(`共生成 ${newBlocks.length} 個色塊`);
    } catch (e) {
      toast.error('生成失敗');
    }
  }
        ...
        <td className="sticky left-0 z-10 border border-gray-200 px-2 py-1 min-w-[140px]" style={{ backgroundColor: rowBg }}>
          <div className="flex items-center justify-between gap-1">
            ...
            <div className="flex gap-0.5">
              <button onClick={() => handleShiftBatch(group.id, -1)} className="p-0.5 hover:bg-gray-100 rounded text-gray-400 hover:text-gray-700" title="提前1天">
                <ChevronLeft className="w-3 h-3" />
              </button>
              <button onClick={() => handleShiftBatch(group.id, 1)} className="p-0.5 hover:bg-gray-100 rounded text-gray-400 hover:text-gray-700" title="延後1天">
        ...
        {dateRange.map(date => {
          ...
          return (
            <td
              ...
              style={{ backgroundColor: cfg.bg, borderColor: cfg.border, width: `${cellWidth}px`, minWidth: `${cellWidth}px`, padding: '2px 3px', verticalAlign: 'middle' }}
            ...
            >
              <div className="text-center leading-tight">
                ...
                {block?.flightNumber && (
                  <div className="text-[8px]" style={{ color: cfg.text, opacity: 0.8 }}>{block.flightNumber}</div>
                )}
              </div>
            </td>
          );
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      {isHomepageMode && !pid && (
        <div className="flex flex-col items-center justify-center flex-1 text-gray-400">
          ...
          <div className="text-sm text-gray-400 mb-4">點擊上方「請選擇項目」下拉框選擇一個項目</div>
          {allProjects.length === 0 && (
          ...
          <table className="border-collapse text-xs" style={{ tableLayout: 'fixed' }}>
            ...
            <tbody>
              {primaryGroups.length > 0 && (
                <>
                  <tr>
                    <td colSpan={dateRange.length + 1} className="bg-blue-50 border border-gray-200 px-3 py-0.5" style={{ fontSize: '10px', color: '#1e40af', fontWeight: 600 }}>
              ...
              {secondaryGroups.length > 0 && (
                <>
                  <tr>
                    <td colSpan={dateRange.length + 1} className="bg-orange-50 border border-gray-200 px-3 py-0.5" style={{ fontSize: '10px', color: '#9a3412', fontWeight: 600 }}>
              ...
              <tr className="bg-gray-50">
                <td className="sticky left-0 z-10 bg-gray-50 border border-gray-200 px-2 py-1 text-xs font-semibold text-gray-600">深圳住宿人數</td>
                {dailyStats.map(({ date, szCount }) => (
                  <td key={date} className="border border-gray-200 text-center" style={{ backgroundColor: szCount > 0 ? '#DAEEF3' : '#F9FAFB', padding: '2px 0' }}>
                    {szCount > 0 && <span className="text-[10px] font-bold text-blue-800">{szCount}</span>}
                  </td>
                ))}
              </tr>
              <tr className="bg-gray-50">
                <td className="sticky left-0 z-10 bg-gray-50 border border-gray-200 px-2 py-1 text-xs font-semibold text-gray-600">香港住宿人數</td>
                {dailyStats.map(({ date, hkCount }) => (
                  <td key={date} className="border border-gray-200 text-center" style={{ backgroundColor: hkCount > 0 ? '#BDD7EE' : '#F9FAFB', padding: '2px 0' }}>
                    {hkCount > 0 && <span className="text-[10px] font-bold text-blue-900">{hkCount}</span>}
                  </td>
                ))}
              </tr>
            </tbody>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-2">
            ...
            <div>
              ...
              <Input type="number" min={0} max={templateHkDays - 1} className="h-7 text-xs mt-1" value={templateExchangeOffset} onChange={e => setTemplateExchangeOffset(Number(e.target.value))} />
            </div>
