...
- [x] **場館約束規則UI**：在行程點編輯表單中集成約束檢測，實時顯示警告提示

#### 模塊三：行程模板- [x] 模板創建功能（從現有團組保存為模板）
- [x] 模板預覽功能
- [x] **模板套用API**：applyTemplate API已實現
- [x] **模板套用UI**：在GroupNew中添加模板選擇器
...
#### 模塊六：學校交流管理
- [x] **學校交流API**：schoolExchanges router已實現（create/list/delete）
- [x] **學校交流UI**：在GroupDetail中添加「學校交流」Tab，支持添加/查看/刪除交流記錄
- [ ] **交流狀態管理**：添加交流狀態字段（待確認/已確認/已完成）

#### 模塊七：現有功能優化（P0級別）
- [x] 團組管理歸屬到項目下
- [x] **行程時間軸優化**：增加上午/下午/晚上時段劃分（橙色/藍色/紫色標籤）
- [x] **食行卡片升級**：升級為項目級餐飲統籌視圖，一覽所有團組的餐飲安排矩陣
